...
namespace DotNetKoans.CSharp
{
	public class AboutGenericContainers : Koan
	{
		...
		public void Boxing()
		{
            ...
            //Richter's book if you want details). 
            // *** the important part:
            //This subject comes up because every time you put a value type into an 
		...
		public void PeekingIntoAQueue()
		{
			...
			queue.Enqueue("two");
			Assert.Equal("one", queue.Peek()); // .Peek shows you beginning of queue, so what you add w/ Enqueue doesn't matter
		}
		[Koan(18)]
		...
		public void TryGetValueForMissingData()
		{
			...
			dictionary["one"] = "uno";
			string result;
			if (!dictionary.TryGetValue("two", out result)) // if dictionary["two"] != result, result = "dos"
			{
				result = "dos";
			}
			Assert.Equal("dos", result);
		...
		public void f()
		{
			...
			Assert.Equal(26, one["jenny"]); // one["jenny"] = 26 created by foreach
